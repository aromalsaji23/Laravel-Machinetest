...
<div class="max-w-[1440px] mx-auto px-6 py-8 md:p-10">
    ...
    <div class="flex justify-between items-center mb-8">
        ...
        <?php if(auth()->user()->isAdmin()): ?>
            <a href="<?php echo e(route('tasks.create')); ?>" class="bg-[#3b82f6] hover:bg-blue-600 transition-colors text-white px-5 py-2.5 rounded-lg font-medium text-sm flex items-center shadow-lg shadow-blue-500/20">
    ...
    <!-- Filter Bar -->
    <form method="GET" action="<?php echo e(route('tasks.index')); ?>" class="flex flex-wrap gap-4 mb-4 items-center w-full max-w-4xl">
        <!-- Search -->
        <div class="relative w-full sm:w-64">
            <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-gray-400">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
            </div>
            <input type="text" name="search" value="<?php echo e($filters['search'] ?? ''); ?>" placeholder="Search Filter Task" class="filter-input w-full pl-10 pr-4 py-2 bg-white text-gray-800 border-0 focus:ring-2 focus:ring-blue-500 text-sm font-medium h-10 shadow-sm" onchange="this.form.submit()">
        </div>
        ...
        <div class="relative w-full sm:w-44">
            <select name="assigned_to" class="filter-input w-full pl-4 pr-10 py-2 bg-white text-gray-800 border-0 appearance-none focus:ring-2 focus:ring-blue-500 text-sm font-medium h-10 shadow-sm" onchange="this.form.submit()">
                ...
                <?php $__currentLoopData = $users; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($user->id); ?>" <?php echo e(($filters['assigned_to'] ?? '') == $user->id ? 'selected' : ''); ?>><?php echo e($user->name); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
            <div class="absolute inset-y-0 right-0 flex items-center px-3 pointer-events-none text-gray-500">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
        ...
        <div class="relative w-full sm:w-40">
            <select name="priority" class="filter-input w-full pl-4 pr-10 py-2 bg-white text-gray-800 border-0 appearance-none focus:ring-2 focus:ring-blue-500 text-sm font-medium h-10 shadow-sm" onchange="this.form.submit()">
                ...
                <?php $__currentLoopData = \App\Enums\TaskPriorityEnum::cases(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $priority): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($priority->value); ?>" <?php echo e(($filters['priority'] ?? '') === $priority->value ? 'selected' : ''); ?>><?php echo e($priority->label()); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
            <div class="absolute inset-y-0 right-0 flex items-center px-3 pointer-events-none text-gray-500">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                ...
                <?php $__empty_1 = true; $__currentLoopData = $tasks; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $task): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="task-card bg-white p-5 text-gray-800 flex flex-col h-full border border-gray-100">
                        ...
                        <div class="flex items-center gap-2 mb-4">
                            ...
                            ?>
                            <span class="text-[10px] font-bold text-white <?php echo e($priorityColor); ?> px-3 py-1 rounded-[10px] uppercase tracking-wide shadow-sm">
                                Priority: <?php echo e($task->priority->label()); ?>

                            </span>
                        </div>
                        
                        <!-- Description/AI Summary -->
                        <div class="text-[12px] text-gray-500 mb-5 leading-relaxed <?php echo e($task->ai_summary ? 'bg-gray-50 p-3 rounded-xl border border-gray-100' : 'line-clamp-3'); ?>">
                            <?php if($task->ai_summary): ?>
                                ...
                                <?php echo e(Str::limit($task->ai_summary, 120)); ?>
                        ...
                        <div class="text-[12px] text-gray-500 mb-5 space-y-1 font-medium flex-1">
                            <p>Assigned to: <span class="text-gray-700"><?php echo e($task->assignedUser->name ?? 'Unassigned'); ?></span></p>
                            <p>Due: <span class="text-gray-700"><?php echo e($task->due_date->format('Y-m-d')); ?></span></p>
                            <p>AI Priority: <span class="text-gray-700"><?php echo e($task->ai_priority?->label() ?? 'N/A'); ?></span></p>
                        </div>
                        
                        <!-- Bottom Action Row -->
                        <div class="flex justify-between items-center pt-2 mt-auto">
                            <!-- Priority Label mapping at bottom left matching mockup design -->
                            <span class="text-[14px] font-bold text-blue-500 tracking-wide"><?php echo e($task->priority->label()); ?></span>
                            
                            <div class="flex gap-2">
                                <?php if(auth()->user()->can('update', $task)): ?>
                                    <a href="<?php echo e(route('tasks.edit', $task)); ?>" class="px-5 py-1.5 bg-gray-100/80 text-gray-700 font-bold rounded-full text-[13px] hover:bg-gray-200 transition text-center border border-transparent">
                                        Edit
                                    </a>
                                <?php endif; ?>
                                <a href="<?php echo e(route('tasks.show', $task)); ?>" class="px-5 py-1.5 bg-[#3b82f6] text-white font-bold rounded-full text-[13px] hover:bg-blue-600 transition text-center shadow-md shadow-blue-500/20">
                                    View
                                </a>
                            </div>
                        </div>
            ...
            <div class="bg-white rounded-[16px] text-gray-800 shadow-xl overflow-hidden pt-6">
                ...
                <div class="flex flex-col items-center justify-center mb-6">
                    ...
                    <div class="text-center">
                        <h2 class="font-bold text-lg text-gray-900 leading-tight"><?php echo e(auth()->user()->name); ?></h2>
                    </div>
                </div>

                <!-- Navigation List -->
                <div class="flex flex-col w-full text-[15px] font-medium border-t border-gray-100">
                    <a href="<?php echo e(route('tasks.index')); ?>" class="py-3.5 px-6 bg-[#3b82f6] text-white w-full text-left">
                        Tasks
                    </a>
                    <a href="<?php echo e(route('dashboard')); ?>" class="py-3.5 px-6 text-gray-700 bg-white border-b border-gray-100 hover:bg-gray-50 block transition-colors">
                        Dashboard
                    </a>
                    <?php if(auth()->user()->isAdmin()): ?>
                        <a href="<?php echo e(route('dashboard')); ?>" class="py-3.5 px-6 text-gray-700 bg-white border-b border-gray-100 hover:bg-gray-50 flex items-center gap-2 transition-colors">
...
<?php /**PATH C:\Users\ACER\OneDrive\Desktop\advi\Task-Management\task-manager\resources\views/tasks/index.blade.php ENDPATH**/ ?>